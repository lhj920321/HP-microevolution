...
rule all:
	input:
	## bwa to self  genome
		#expand("/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/ngmlr/{sample}/{sample}_2_{bwa_ref_samp}.sort.bam",sample=REP_INDEX,bwa_ref_samp=bwa_ref_samp),
		#expand("/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/ngmlr/{sample}/{sample}_2_{bwa_ref_samp}.sort.bam.bai",sample=REP_INDEX,bwa_ref_samp=bwa_ref_samp),
	##mpileup
		expand("/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/mpileup/{sample}_2_{bwa_ref_samp}.sort.mpileup",sample=REP_INDEX,bwa_ref_samp=bwa_ref_samp)
...
rule ngmlr_run:
	...
	output:
		temp("/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/ngmlr/{sample}/{sample}_2_{bwa_ref_samp}.bam")
	shell:
		"ngmlr -t 30 -x  pacbio -r   {input[0]} -q  {input[1]}  \
      		|  samtools view -bS -bF 4 -  > {output}   "   #
 

rule bam_sort:
	input:
		"/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/ngmlr/{sample}/{sample}_2_{bwa_ref_samp}.bam",
	output:
		"/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/ngmlr/{sample}/{sample}_2_{bwa_ref_samp}.sort.bam",
	shell:
...
rule bam_index:
	input:
		"/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/ngmlr/{sample}/{sample}_2_{bwa_ref_samp}.sort.bam"
	output:
		"/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/ngmlr/{sample}/{sample}_2_{bwa_ref_samp}.sort.bam.bai"
	shell:
...
rule mpileup:
	input:
		...
		"/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/ngmlr/{sample}/{sample}_2_{bwa_ref_samp}.sort.bam.bai"
	output:
		"/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/mpileup/{sample}_2_{bwa_ref_samp}.sort.mpileup"
	log:
		"/shared/liuhj/HP/process/CCS_map2PersonRespectSamp/mpileup/{sample}_2_{bwa_ref_samp}.sort.mpileup.log"
	shell:
		"samtools mpileup -q 20  --reference  {input[0]}  {input[1]}  1>{output} 2>{log}"     
...
rule varscan2_mpileup2snp:
	...
	shell:
		"varscan   mpileup2snp  {input}  --min-coverage 50  --min-reads2  5  --min-avg-qual 20  --min-var-freq  0.02   --output-vcf 1  >{output} "     
