...
def Gnms_stat(CoreGnmF,PersonDiffPoAnno):
	PerSeqsdic = {}
	...
	personNAPosiNum,personDiffPosiNum,personDiffAnnoDic = {},{},{}
	for personID in PerSeqsdic:
		...
		for CorePos in range(0,SeqLen):
			if CorePos+1  not in persRecombiPosi:
				Pers_Bases = {}
	...
	for pers in PerSeqsdic:
		...
		outl = "\t".join([pers,group,str(personDiffPosiNum[pers]),str(StopGainNum),str(StopLostNum),str(otherAnno),str(SynNum),str(MissNum)])
		outs.append(outl)
...
def main():
	...
	outs,outAnno = Gnms_stat(CoreGnmF,PersonDiffPoAnno)
	OUts =[]
	OUts.append("\t".join(["person","group","SNPLoci","StopGainNum","StopLostNum","otherAnno","SynNum","MissNum"]))
	OUts += outs
